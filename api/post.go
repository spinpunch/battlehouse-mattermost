...
func createPost(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-app.Srv.Store.Channel().Get(post.ChannelId, true); result.Err != nil {
		channel = nil
	} else {
		channel = result.Data.(*model.Channel)
	}

	if (channel != nil) && (channel.Type == model.CHANNEL_OPEN) && (bhInviteRe.FindStringSubmatch(post.Message) != nil) {
