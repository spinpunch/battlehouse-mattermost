...
func login(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	user, err := app.AuthenticateUserForLogin(id, loginId, password, mfaToken, rawBcrypt, BHApiSecret, deviceId, ldapOnly)
	if err != nil {
...
func doLogin(c *Context, w http.ResponseWriter, r *http.Request, user *model.User, deviceId string) {
	session, err := app.DoLogin(c.cookieDomain(), w, r, user, deviceId)
	if err != nil {
		c.Err = err
		return
	}
...
func revokeUserBH(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := app.RevokeAllSessions(userId); err != nil {
		c.Err = err
		return
	}
...
func updateUserBH(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if ruser, err := app.UpdateUserAsUserBH(userId, c.IsSystemAdmin(),
		props["new_username"], props["new_email"], props["new_nickname"]); err != nil {
		c.Err = err
		return
	} else {
		c.LogAudit("")
...
func do_uploadProfileImage(userId string, c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := app.SetProfileImage(userId, imageData); err != nil {
		c.Err = err
		return
	}
...
func getProfilesByIds(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// battlehouse.com: this used to disallow caching for admin queries,
