...
func CreateUserBH(user *model.User, siteURL string, skipTutorial bool) (*model.User, *model.AppError) {
	// battlehouse.com - allow API caller to say the email is already verified (by not overwriting it)
	...
	if skipTutorial {
		pref := model.Preference{UserId: ruser.Id, Category: model.PREFERENCE_CATEGORY_TUTORIAL_STEPS, Name: ruser.Id, Value: "999"}
...
func UpdateUserAsUserBH(userId string, asAdmin bool, new_username, new_email, new_nickname *string) (*model.User, *model.AppError) {
	if result := <-Srv.Store.User().UpdateBH(userId, new_username, new_email, new_nickname); result.Err != nil {
		return nil, result.Err
	} else {
